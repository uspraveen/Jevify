...
## 14. Three behaviours jev-bench did not test

jev-bench asks whether a model knows the answer. Three behaviours a production decision model needs
are not isolated by any of its configs, so three tests were added (`scripts/build_b.py`), each built so
that a model cannot pass by having trained on look-alike data:

- **A stated rule** (`legal_rules`, 885 items, Noul): human-written fact patterns from ten LegalBench
  rule-application tasks, with the rule written into the question — diversity jurisdiction (six
  variants), hearsay, personal jurisdiction, the Telemarketing Sales Rule, UCC vs common law.
- **None of the above** (`nota`, 1,000 items, Choice): MMLU and ARC-Challenge test items with an extra
  option, "None of the other options is correct."; in half of them the correct option is removed.
- **An instruction hidden in the input** (`injection`, 800 pairs, Noul): test items from SMS spam,
  toxic comments, BoolQ and FEVER, each as it is and with a sentence planted in the text that names the
  wrong answer, in our own wording (Tev1's training data contains injected comments of a different kind).

| | stated rule: accuracy | "none" when the right option is gone | "none" when it is there | hijack rate | toward "no" |
|---|---|---|---|---|---|
...
*Hijack rate: how much more often a model gives the answer the planted sentence names than on the clean
copy of the same item. "Toward no": planted sentences asking for the harmless-looking answer (not spam,
not toxic, not supported) — the attacker's direction.*

**14.1 Applying a stated rule is Jev's largest lead anywhere in this project: 22 points.** Jev 0.924;
every open model 0.62–0.70, each leaning to "no" (18–39% yes against a 45% base rate). On diversity
jurisdiction — arithmetic and matching once the rule is stated — Jev scores 0.947 and the open models
0.56–0.67. Tev1 trained on 13,500 synthetic rule-policy examples and reaches 0.678: rules generated by
code did not transfer to rules written by lawyers. This is the capability gap that Section 11's
held-out numbers do not show.

**14.2 Fine-tuning taught both 4Bs to notice a missing answer.** With the right option removed, the
untrained 4B picks "none" 48% of the time; Tev1 (trained with "insufficient information" answers) 70%,
and the Jevify Tier 2 model — never shown such an answer — 66%. Tev1 also picks "none" on 14% of items
whose answer is present (Jev 1.6%, Tier 2 4.4%).

**14.3 Fine-tuning made both 4Bs much harder to steer with a planted instruction, and Jev resists the
attacker's direction best.** Hijack rate +0.40 untrained, +0.14 Tev1, +0.16 Tier 2, +0.21 Jev — but
toward "no" Jev moves only +0.03. A planted "not spam" in a real spam message never flipped Jev or the
Tier 2 model (0 of 30 each), against 10 of 30 for the untrained 4B and 4 of 30 for Tev1 (small counts;
the rates over all 800 pairs are not). Jev does follow a planted "spam" in an ordinary message (+0.29
toward "yes"): over-cautious, not exploitable.

*(`results/b/` — README, `tables.md` with every task and source, `report.json`; predictions on the Hub.)*
